 ...
 */
@Controller
@RequestMapping("/hcountry")
public class HcountryAction extends BaseAction {
	...
	public String list(ModelMap modelMap ,Integer status) throws Exception{
		...
		List<Hcountry> list = hcountrySvc.findListBy(status);
	...
	public String updateStatus(String backIds) throws Exception{
		...
		if(!parameterSvc.getNightAuditState()){
			hcountrySvc.updateStatusById((StringUtils.isNotBlank(backIds) ? CommonUtil.array_unique(backIds.split(",")) : null)  , 0 );
	...
	public String saveOrUpdateOrDel(String params , String delIds) throws Exception{
		...
		try {
			if(!parameterSvc.getNightAuditState()){
				List<Hcountry> hcountrys = JSON.parseArray(params, Hcountry.class) ;
				hcountrySvc.saveOrUpdateOrDel(hcountrys , delIds);
				jsonObj.put("success", true);
	...
	public String findBy(Integer id , String codeId){
		JSONObject jsonObj =  new JSONObject();
		Hcountry entity = hcountrySvc.get("codeId",codeId) ;
		if(id == null){
			if(entity !=null){
				jsonObj.put("success", false);
			}else{
