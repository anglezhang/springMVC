...
//		flexGridP = createFlexGridP("#divchangetoroom", viewP);
		...
		if(pointtimes == 0){
			$("#radio_check").val("P");
			var room_id = $("#guestinfo_room_id").val();
			$("#room_idP").val(room_id);
			searchToData(false);
		}
		pointtimes++;
	});
...
function provideRoomCard(position){
	...
	guest_room_utils.RegisterUserInfo($("#userid").val(), $("#username").val());
	//是否发房卡
	var hairRoomCard = $("#guestinfo_hairRoomCard").val()==='true' ? true : false; 
	guest_room_utils.ScanOutIdCard(creatCardInfo(guests),hairRoomCard);
	//var a = idcard.runADELCard(checkIdAndroomId);
}

var pointtimes = 0;
function enter(){
	$(".chaRomPoint1").attr("checked", "checked");
	giveUp();
	$("#radio_check").val("H");
	$(".chaRom1").css("display", "block");
	$(".chaRom2").css("display", "none");
	var room_id = $("#guestinfo_room_id").val();
	$("#room_idA").val(room_id);
	searchToDataA();
	$("#cr_div").fadeIn();
} 