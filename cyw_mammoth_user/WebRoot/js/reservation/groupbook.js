...
$(function(){
    ...
    grid = new wijmo.grid.FlexGrid('#gptable',{
        ...
        columns: [
             { header: '序号',binding: 'number', minWidth:40, width: 50,isReadOnly: true, allowMerging: true,cssClass:'fg_column_readOnly'}, 
             { header: '订单号',binding: 'bookList',minWidth:60, width: 100,allowMerging: true }, 
             {header: '团代码',binding: 'grpId', minWidth:60, width: 100,allowMerging: true}, 
             {header: '团名',binding: 'grpName',minWidth:60, width: 93,allowMerging: true}, 
        //----------------bookroom info----------------------
        {
            header: '房类',
            binding: 'codeNamec',
...
function consumeShow(){
	var checkId=$("#skyd input[name='checkId']").val();
...
$("#otherInformation").click(function(){
	...
	$.ajax({
	   ...
	   success: function(msg){
		   ...
		   $("#otherInfoForm input[name='bookTime']").val(formatterDate(msg.obj.bookTime,'yyyy-MM-dd'));
...
function otherInformationFunc(){
	...
	$.ajax({
	   type: "POST",
	   url: path+"/group/getGroupDocByCheckId.do",
	   data: {checkId:checkId},
	   dataType:'json',
	   success: function(msg){
		   //setJSONToForm($("#otherInfoForm"),msg.obj);
...
function goSearch(code) {
    ...
    $.ajax({
        type: "POST",
        url: path + "/group/groupdocbooklist.do",
...
//------------------------------------------确定add Action---------------------------------------------
	    ...
	    var msg="留房数据不正确";
	    //修改预计人数为零仍能保存的问题
	    if(parseInt($("#gstNum").val()) <= 0){
			altWaringMsg("预计人数必须大于0。");
			return false;
	    }
	    if (checkIdval == "true") {
	    ...
	    if (true) {
	        //修改
	        if (checkIdval == "true") {
	            var url = path + "/group/update.do";
	        } else {
	            //保存
	            var url = path + "/group/save.do";
	        }
	        $.ajax({
	            url: url,
	            type: "post",
...
function goView(checkId1) {
	...
	$("#theBookList").addClass("gry_9").attr("readonly","readonly");
	
	$("#billId").addClass("gry_9").attr("readonly","readonly");
                            ...
                            if (panel.cellType == wijmo.grid.CellType.Cell) {
                                ...
                                if (r == editIndex) {
                                    ...
                                    case 'buttons':
                                    	...
                                    	var cellData=editBookRoomFlexGrid.getCellData(r,10);//抵达数
                                    	var ddd=editBookRoomFlexGrid.getCellData(r,4);
                                    	if(typeof(editBookRoomFlexGrid.getCellData(r,1))!="undefined" && editBookRoomFlexGrid.getCellData(r,1)!=null){
                                            ...
                                            '&nbsp;&nbsp;' + 
                                            '<a href="javascript:void(0)" class="btn btn-default btn-sm ttrr" onclick="editRow(' + r + ')">' + 
