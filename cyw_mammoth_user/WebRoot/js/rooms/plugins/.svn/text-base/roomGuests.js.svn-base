var guestGrid,tag,maxNum,flag,isAdd,isEdit=false,reachDate,leaveDate,roomTag,currentTxt,accountGrid,_nowprice,checkOutGrid,preAuthorizationGrid,transferAccountGrid,transferArr=[],customAccountsGrid,LeavingHotelGrid,checkInTag=true,moenyId;
$(function(){
		...
		/*房价列表二级弹出*/
		$("#pricesList,#priceList_2").click(function() {
		...
		$("#customAccounts").click(function(){
			...
			$.ajax({
				url : '/guest/guestguestAccountInfo.do',
				type : 'POST',
				cache : false,
...
function getDetail(item,position,type){
	var url = '/guest/guestDocDetail.do';
	$.ajax({
		url :url,
		type : 'POST',
		cache : false,
		data:{check_id:item.check_id,room_id:item.room_id,with_id:item.with_id,roomIds:item.roomIds,reachDate:item.reachDate,leaveDate:item.leaveDate},
		async:false,
		dataType:'json',
		success : function(data, textStatus, jqXHR) {
			//表单加载
            ...
            $("#roomList li").each(function(){
            	if(item.roomIds){
            		if(item.roomIds.split(',')[0] == $(this).text()){
                		$(this).addClass('roomListLi');
                	}
            	}
...
function loadConsume(checked){
	...
	$.ajax({
		...
		success:function(data){
			if(data.consumes){
				for(var i = 0;i<data.consumes.length;i++){
					$("#consumeA_ID").append("<li id='"+data.consumes[i].codeId+"' ondblclick='consumeDblclick(this)' tag='false' onclick='consumeClick(this)'>"+data.consumes[i].codeNamec+"</li>");
				}
			}
			if(data.blist){
				for(var i = 0;i<data.blist.length;i++){
					$("#consumeB_ID").append("<li id='"+data.blist[i].codeId+"' ondblclick='consumeDblclick(this)' tag='false' onclick='consumeClick(this)'>"+data.blist[i].codeNamec+"</li>");
				}
			}
			...
			success : function(data, textStatus, jqXHR) {
				if(data){
					...
					if(data.info){
						...
						item.check_id = arr[0].split(',')[0];
						item.room_id = arr[0].split(',')[1];
						item.with_id = data.withId;
					}
					// 调用 杨军鹏 的方法
					getDetail(item,null,type);
					if(checkInTag){
						if(type==1){
							roomTag = item.roomIds.split(',')[0];
					...
					$(".alertDivBg").fadeIn();
					$(".checkDetailsDiv").fadeIn();
					$(".checkDetailsDiv").css('z-index',11);
...
function getItem(type){
	...
	if(checkInTag){
		if(type==1){
			var reachDate = $("#roomssearchvo_startdate_minDate").val();//开始日期
			var leaveDate = $("#roomssearchvo_old_enddate").val();//截至日期
			var roomIds = rooms_model_cach['roomId'];
			item.roomIds = roomIds;
			item.reachDate = reachDate;
			item.leaveDate = leaveDate;
		}else if(type==2){
			var liObj = $("[isshow='true']").parent();
			var withID = liObj.attr("guest-withid");
			var checkId = liObj.attr("guest-checkid");
			var roomIds = liObj.attr('room-id');;
			item.check_id = checkId;
			item.with_id = withID;
			item.room_id = roomIds;
		}
	}else{
		item = guestGrid.itemsSource.currentItem;
	}
