function PageSystem(count, divID, grountCount, callBack) {
     ...
     $(this.pageDivObj).addClass("page-link");
...
//          _span.style.cssText = "margin-left:10px";
    ...
    //解决Ie 与 火狐 event兼容	
       ...
       that.reViewActivePage(srcElement);
       that.query();
    };
    this.reViewActivePage = function(obj_a) {
      //重新渲染当前页视图
      var actA = obj_a; //当前被点击的 a对象
      var ap = actA.parentNode; //获得当前a容器span对象
      //还原当前页视图
       var _a = document.createElement("A");
       // _a.href = "#";
        _a.onclick = this.reView;
        _a.innerHTML = that.currentActiveSpan.innerHTML;
        $(that.currentActiveSpan).removeClass("current");
        that.currentActiveSpan.innerHTML = "";
        that.currentActiveSpan.appendChild(_a);
       //渲染新的当前页视图
       that.currentActiveSpan = ap; //切换当前活动页容器
       if(actA.innerHTML=="<"){actA.innerHTML=that.currentPage;}
       var curPage = parseInt(actA.innerHTML);
       $(that.currentActiveSpan).addClass("current");
       that.currentActiveSpan.removeChild(actA);
       that.currentActiveSpan.innerHTML = curPage;
    ...
    this.toNextGroup = function() {
       ...
       if(that.currentPage == that.currentMax) {//点击的页码为当前组最大页码，当go 下一组
         if(that.currentPage != that.endPage) { //如果点了未页当然不会再有下一组啦！
           ...
           // this.createHomePage();
            this.createPrePage();
            that.currentActiveSpan.innerHTML = that.currentPage;
            $(that.currentActiveSpan).addClass("current");
...
//       _span.style.cssText = "margin-left:10px";
     ...
     this.next = function() {
         //下一页
    	 if(that.pages==that.currentPage)return;
    	 if(!that.toNextGroup());
         that.reViewActivePage($(".page-link .current ").next("span").find("a")[0]);
//         that.currentPage = that.currentPage+1;
     ...
     this.pre = function() {
       //上一页
    	    ...
    	    if(!that.toPreGroup()) ;
    	     that.reViewActivePage($(".page-link .current ").prev("span").find("a")[0]);
//        	 that.currentPage = that.currentPage-1;
        	 that.query();
     };
...
//       _a.href = "#";
     ...
     this.createHomePage = function() {
       ...
       if(this.currentPage==1){
    	   $(homeSpan).addClass("current");   
       }else{
    	   $(homeSpan).removeClass("current");
       }
       var _a = document.createElement("A");
//       _a.href = "#";
