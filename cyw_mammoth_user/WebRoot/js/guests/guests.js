var view,grid,guestGrid,tag,maxNum,flag,isAdd=false,isEdit=false,reachDate,leaveDate,roomTag,currentTxt,accountGrid,_nowprice,checkOutGrid,preAuthorizationGrid,transferAccountGrid,transferArr=[],customAccountsGrid,LeavingHotelGrid,moenyId;
var consumeArr = [];
$(function(){
	...
	//房间号及帐号输入限制
	$("#searchRoomNum").inputmask('Regex',{regex:"[0-9]*$"});
	...
	$(".userTab1").click(function(e) {
		var item={};
		var isshow = document.getElementById("checkDetailsDiv").style.display;
		if(!isshow || isshow=='none'){
			item = grid.itemsSource.currentItem;
		}else{
			if($("#account_namec").val()){
				item.check_id = $("#account_namec").val();
				item.with_id = $("#account_namec option:selected").attr('data-withid');
				item.room_id = $("#account_roomid").val();
			}else if($("#account_namee").val()){
				item.check_id = $("#account_namee").val();
				item.with_id = $("#account_namee option:selected").attr('data-withid');
				item.room_id = $("#account_roomid").val();
			}
		}
		if(item){
	...
	$("#pricesList").click(function() {
		 ...
		 var withId = $("#form1 #withId").val();
		 // 当前选中的房间号
		 var currentRoomId = $("#roomId").val() ;
		 $.ajax({
		    	url:'/guest/findRoomPriceListBy.do',
		    	async:false,
		    	data:{checkId:checkId , withId:withId},
		    	dataType:'json',
		    	success:function(data){
		    		if(data){
		    			var listJson = data.roomPriceList ;
		    			var hotelDate = data.hotelDate ;
		    			var li_html = "<li class=\"fontWeight\"> 房间列表</li>" ;
		    			var currentRoomTypeName = "" ;
		    			var currentReachDate = "" ;
		    			var currentLeaveDate = "" ;
		    			var currentStandPrice = "" ;
		    			var currentDoPrice = "" ;
		    			
		    			for(var i=0;i<listJson.length;i++){
		    				var currObj = listJson[i] ;
		    				var roomId = currObj.roomId ;
		    				var roomTypeName = currObj.roomTypeName ;
		    				var doPrice = currObj.doPrice ;
		    				if(roomId.trim() == currentRoomId.trim()){
		    					currentRoomTypeName = roomTypeName ;
		    					currentReachDate = currObj.reachDate ;
		    					currentLeaveDate = currObj.leaveDate ;
		    					currentStandPrice = currObj.standPrice ;
		    					currentDoPrice = doPrice ;
		    					
		    					li_html += "<li id=\""+roomId.trim()+"\"> " ;
		    					li_html += "	<a style=\"background:#e4f1d0\" href=\"javascript:fillRoomPriceTableBy('"+currObj.reachDate+"','"+currObj.leaveDate+"','"
		    												+currObj.standPrice+"','"+doPrice+"','"+roomId+"','"+roomTypeName+"','"+hotelDate+"')\">"+currObj.roomId+"</a>";
		    					li_html += "</li> " ;
		    				}else{
		    					li_html += "<li id=\""+roomId.trim()+"\"> " ;
		    					li_html += "	<a href=\"javascript:fillRoomPriceTableBy('"+currObj.reachDate+"','"+currObj.leaveDate+"','"
		    												+currObj.standPrice+"','"+doPrice+"','"+roomId+"','"+roomTypeName+"','"+hotelDate+"')\">"+currObj.roomId+"</a>";
		    					li_html += "</li> " ;
		    				}
		    			...
		    			$("ul#pricesList_roomList").html(li_html);
		    			fillRoomPriceTableBy(currentReachDate, currentLeaveDate, currentStandPrice, currentDoPrice , currentRoomId , currentRoomTypeName,hotelDate);
		    		}
		    	}
		});
		$(".pricesList").fadeIn();
...
//	});
	...
	$("#accountedFor").click(function(){
		...
		$.ajax({
			...
			success : function(data, textStatus, jqXHR) {
				...
				if(data.consumes){
					for(var i=0;i<data.consumes.length;i++){
						$("#accountConsume").append('<option data-svcRate="'+data.consumes[i].svcRate+'" value="'+$.trim(data.consumes[i].codeId)+'">'+data.consumes[i].codeNamec+'</option>');
					}
				}
				if(data.exchanges){
					for(var i=0;i<data.exchanges.length;i++){
						$("#accountExchange").append('<option value="'+$.trim(data.exchanges[i].codeId)+'">'+data.exchanges[i].codeNamec+'</option>');
					}
				}
	 ...
	 * grid数据加载
	 */
    grid = new wijmo.grid.FlexGrid('#guestGrid', {
     	  autoGenerateColumns: false,
    	  columns: [
           { header: '序号', binding: 'sortNum',align:'center' , width:40,minWidth:40}, 
           { header: '房号', binding: 'room_id',align:'center',width:40,minWidth:60}, 
           { header: '房类', binding: 'code_namec',width:80,minWidth:80}, 
           { header: '中文名', binding: 'gst_namec' ,width:80,minWidth:80},
           { header: '英文名', binding: 'gst_namee' ,width:80,minWidth:80}, 
           { header: '团代码', binding: 'grp_id',width:80,minWidth:80},
           { header: '团名', binding: 'grp_name',width:80,minWidth:80},
           { header: '抵店日期', binding: 'reach_date',align:'center',format:'yyyy-MM-dd',width:80,minWidth:100},
           { header: '离店日期', binding: 'leave_date',align:'center',format:'yyyy-MM-dd',width:80,minWidth:100},
           { header: '合约单位', binding: 'namec',width:80,minWidth:80},
           { header: '状态', binding: 'chkStat',align:'center',width:60,minWidth:60},
           { header: '账号', binding: 'billa_id',width:40,minWidth:63},
           { header: '登记人', binding: 'oper_name',width:60,minWidth:80},
           { header: '登记号', binding: 'check_id',width:'*',minWidth:70}
       	],
        //itemsSource: view,
        headersVisibility:wijmo.grid.HeadersVisibility.Column,
...
//			var flag = true;
//			$.ajax({
//				url:'checkPaymanFlag.do',
//				data:{withId:$("input[name=withId]").val(),check_id:$("#checkId").val()},
//				async:false,
//				dataType:'json',
//				success:function(data){
//					flag = data.success;
...
function scanCard(data){
	...
	var rtnBool = idcard.runIDCard(data);
	return rtnBool;
}
function isEnabled(orgBookStat){
	 if(orgBookStat=='O'){
    	return false;
    }else{
    	return true;
...
function closeWindow(){
	if($("#guestDetailQuit").hasClass('disabledAButton')) return;
 	$(".alertDivBg").fadeOut();
	$(".checkDetailsDiv").fadeOut();
	var chkStat = $("#chkStat").val();
	isEnabled(chkStat) ? doSearch('#') : doSearch('*');
}
/**
 * 点击右上叉号退出
 */
...
//		if(data.detail.gstOriId!='*')initCodes(data.detail.inType,data.detail.gstOriId);
		...
		{
			$("#moveDivAlert").html("客单详情（离店）");
		}else
		{
			$("#moveDivAlert").html("客单详情（在住）");
		}
		...
		$("#room_id").val(data.detail.roomId);
...
function searchByAccount(e){
	...
	if(keynum==13){
		...
		$("#account_namec option").remove();
		$("#account_namee option").remove();
		if($("#searchaAccountId").val()=='')return;
		$.ajax({
			url:'searchByAccount.do',
			...
			success:function(data){
				if(data){
		            ...
		            $("#roomList2 li").each(function(){
		            	if(data.detail.roomId == $(this).text()){
		            		$(this).addClass('roomListLi');
		            	}
		            });
		            $("#roomList2 li").click(function(){
 ...
 * @param item
 */
function getDetail(item,position){
	$.ajax({
		url :'guestDocDetail.do',
		type : 'POST',
		cache : false,
		data:{check_id:item.check_id,room_id:item.room_id,with_id:item.with_id},
		async:false,
		dataType:'json',
		success : function(data, textStatus, jqXHR) {
			//表单加载
			...
			if(!position){
				if(data.guests){
					for(var i=0;i<data.guests.length;i++){
						if(data.guests[i].check_id == item.check_id){
							position = i;
							break;
						}
					}
            ...
            for(var i=0;i<data.rooms.length;i++){
					$("#roomList").append("<li><a href='javascript:;' onclick='loadGuest("+data.rooms[i].room_id+")'>"+data.rooms[i].room_id+"</a></li>");
					$("#roomList2").append("<li><a href='javascript:;' onclick='loadAccountInfoByRoom("+data.rooms[i].room_id+")'>"+data.rooms[i].room_id+"</a></li>");
					$("#priceRoomList").append("<li><a href='javascript:;'>"+data.rooms[i].room_id+"</a></li>");
            }
...
function loadConsume(checked){
	...
	$.ajax({
		...
		success:function(data){
			...
			//记录当前guestdoc bookStat 
			$("#chkStat").val(data.chkStat);
		}
	});
}
...
function loadAccountConsume(checked){
	...
	$.ajax({
		...
		success:function(data){
			if(data.consumes){
				for(var i = 0;i<data.consumes.length;i++){
					$("#account_consumeAID").append("<li id='"+data.consumes[i].codeId+"' ondblclick='accountConsumeDblclick(this)' tag='false' onclick='consumeClick(this)'>"+data.consumes[i].codeNamec+"</li>");
				}
			}
			if(data.blist){
				for(var i = 0;i<data.blist.length;i++){
					$("#account_consumeBID").append("<li id='"+data.blist[i].codeId+"' ondblclick='accountConsumeDblclick(this)' tag='false' onclick='consumeClick(this)'>"+data.blist[i].codeNamec+"</li>");
				}
			}
...
function getAccountDetail(item){
	...
	$.ajax({
		...
		type : 'POST',
		cache : false,
		data:{check_id:item.check_id,room_id:item.room_id,with_id:item.with_id,billId:billId,billType:bill_type,showType:show_type,okFlag:okFlag},
		async:false,
		dataType:'json',
		success : function(data, textStatus, jqXHR) {
			loadAccountInfo(data.detail);
...
function showDetails(){
	...
	$(".alertDivBg").fadeIn();
	$(".checkDetailsDiv").fadeIn();
	$("#crdPhoto").attr('src',basePath
		+'guest/loadImage.do?tableName=Citizen&colName=id_bmp&where=where Check_ID='
		+item.check_id+'&r='+Math.random());*/
	/*var startDate = $("#roomssearchvo_startdate_minDate").val();//开始日期
	var endDate = $("#roomssearchvo_old_enddate").val();//截至日期*/
	var url = basePath + "/guestdetail/getGuestdocInfo.do";
	var sendData = {'checkId':item.check_id};
...
function getNextDay(day){
	var nextDay = getDateType(day);
	//当前日期的毫秒数 + 天数 * 一天的毫秒数
	var nextDayMills = nextDay.getTime() + 1 * 24 * 60 * 60 * 1000;
	var result = new Date(nextDayMills);
	var month = (result.getMonth() + 1) ;
	var day = (result.getDate()) ;
	return (result.getFullYear() + "-" + (month.toString().length==1 ? "0"+month : month) + "-" + (day.toString().length==1 ? "0"+day : day));
}
...
function fillRoomPriceTableBy(reachDate , leaveDate , standPrice , doPrice , roomId ,roomTypeName,hotelDate){
	var nextDay = getNextDay(reachDate);
	var tr_html = "" ;
	var i = 1 ;
	var totalPrice = 0 ;
	do{
		tr_html += "<tr>" ;
		tr_html += "    <td width=\"10%\" style=\"color:gray;background-color:#EAEAEA;\">"+i+"</td>" ;
		tr_html += "    <td width=\"35%\" style=\"color:gray;\">"+nextDay+"</td>" ;
		tr_html += "    <td width=\"14%\" style=\"color:gray;\">"+getWeekBy(getDateType(nextDay).getDay())+"</td>" ;
		tr_html += "    <td width=\"20%\" style=\"color:gray;\">"+toDecimal2(standPrice)+"</td>" ;
		if(getDateType(hotelDate)>getDateType(nextDay)){
			tr_html += "    <td style=\"color:gray;\">"+toDecimal2(doPrice)+"</td>" ;
		}else{
			tr_html += "    <td>"+toDecimal2(doPrice)+"</td>" ;
		}
		tr_html += "</tr>" ;
		i++ ;
		nextDay = getNextDay(nextDay);
		totalPrice = parseFloat(totalPrice) + parseFloat(doPrice) ;
	}while (getDateType(nextDay)<=getDateType(leaveDate))
	$("#doPriceTotal").html(toDecimal2(totalPrice));
	$("#roomId_roomType_h5").html(roomId +" | "+roomTypeName);
	$("#roomPriceTable tbody").html(tr_html);
	
	$("ul#pricesList_roomList li a").css("background" , "") ;
	$("ul#pricesList_roomList li#"+roomId.toString().trim() + " a").css("background" , "#e4f1d0") ;
}
/*function startScanCard(){
	...
	$(".alertDivBg3").fadeOut();
	$(".checkDetailsDiv").fadeIn();
	$("#crdPhoto").attr('src',basePath+'guest/loadImage.do?tableName=Citizen&colName=id_bmp&where=where Check_ID='+item.check_id+'&r='+Math.random());
}*/
function closeRoomPriceList(){
	$(".alertDivBg2").fadeOut();
	$(".pricesList").fadeOut();
}