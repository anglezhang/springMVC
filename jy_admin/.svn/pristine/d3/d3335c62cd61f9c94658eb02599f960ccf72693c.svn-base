...
<div style="display: none">
	<input type="checkbox"  id="lookbrand" name="infoDepotPosition"/>
</div>
	<div class="formBar">
		<ul>
			<li>
				<div class="button"><div class="buttonContent"><button type="button" id="buttonlook" multLookup="infoDepotPosition" >选择带回</button></div></div>
			</li>
			<li>
		...
		if(null!=bId&&bId!=""){
			...
			if(nodes.length > 0 && nodes[0].id > 0){
				if(nodes[0].isParent){
					...
					var s="{id:'"+nodes[0].id+"',name:'"+typeName+nodes[0].name+"'}"
					$("#lookbrand").val(s);
