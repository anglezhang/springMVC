...
<div class="pageContent">
	<table class="table" width="100%" layoutH="145">
		...
		<tbody>
			<c:forEach items="${pagerIshander.list}" var="customerAppoit" varStatus="s">
				<tr>
					<td>
	...
	<div class="panelBar">
		<div class="pages">
			...
			<span>条，共${pagerIshander.totalCount}条</span>
		</div>
		
		<div class="pagination" targetType="navTab" totalCount="${pagerIshander.totalCount}" numPerPage="${pagerIshander.numPerPage}" pageNumShown="10" currentPage="${pagerIshander.pageNum}"></div>
	</div>
	<script>
		$("select[name='numPerPage']").val('${pagerIshander.numPerPage}');
	</script>
</div>
