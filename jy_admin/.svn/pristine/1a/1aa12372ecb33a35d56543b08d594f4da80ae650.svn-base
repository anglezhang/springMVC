	...
	<div class="panelBar">
		<ul class="toolBar">
			<li>
				...
				<a class="add" href="${pageContext.request.contextPath}/apporder/addcustomer.do?dialogId=orderaddcustomer" target="dialog"  width="530" height="300" mask="true" rel="orderaddcustomer" ><span>添加</span></a>
				</li>
				</tag:auth>	
		</ul>
	</div>
		...
		<tbody>
			<c:forEach items="${pager.list}" var="infoCustome" varStatus="status">
				<tr>
					...
					<td align="center">
						<a class="btn btn-xzdh" href="javascript:$.bringBack({customerId:'${infoCustome.customerId}',customerName:'${infoCustome.customerName}',tel:'${infoCustome.tel}'}),setcarLookVal({customerId:'${infoCustome.customerId}',customerName:'${infoCustome.customerName}',tel:'${infoCustome.tel}'})" title="选择带回"></a>
