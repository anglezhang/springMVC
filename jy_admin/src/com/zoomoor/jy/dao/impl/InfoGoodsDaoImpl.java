...
public class InfoGoodsDaoImpl extends BaseDaoImpl<InfoGoods, Integer> implements
	...
	public Pager getPage(String queryGoodsCode, String queryName,String queryBrand,Integer brandId, int pageNo,
		...
		if(StringUtils.isNotEmpty(queryName)){
			f.append(" and  (bean.name like:queryName or bean.pinyin like:queryName)");
	...
	*/
	...
	public Pager getLookupPage(Pager pager, String queryGoodsCode,
		...
		if(brandId != null){
			...
			String querySQL = "SELECT b.id FROM info_brand b "
					+ " where b_level=1 and id=" + brandId 
					+ " UNION "
					+ " SELECT b.id from info_brand b where b_level=2 ";
			Query query = this.getSession().createSQLQuery(querySQL);
			List<Integer> list = query.list();
			if(list != null && list.size() > 0){
				StringBuffer sb = new StringBuffer(" OR bean.infoBrand.id in( ");
				for(Integer id:list){
					sb.append(id + ",");
				}
				sb.append("-1)");
